    ...
    public function Run($ScanID, $WebsiteLink) {
        ...
        foreach ($this->Ports as $Port) {        
            ...
            if (is_resource($PortConnection)) {
                $PortStatus = $this->PortStatuses[0];
                fclose($PortConnection);
    		} else {
                $PortStatus = $this->PortStatuses[1];
	    	}
            ...
            $ResponsePack = array(
                ...
                "port_service" => !empty(getservbyport($Port, $this->ProtocolType)) ? getservbyport($Port, $this->ProtocolType) : "unknown"
            );

            $this->Database->Insert("portscanner_results", $ResponsePack);
        }
